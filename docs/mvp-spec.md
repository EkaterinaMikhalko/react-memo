...
При выборе легкого режима у игрока есть 3 жизни. Игра заканчивается только после трех ошибок.
...
На сложном уровне игры есть лидерборд, на который попадает любой выигравший игрок.

### Суперсилы

В игре реализована суперсила "Прозрение", которая позволяет открыть все карты на 5 секунд один раз за игру.

### Достижения

При прохождении сложной игры в обычном режиме (с одной жизнью) или без применения суперсилы в лидерборде отображается соответствующее достижение.
